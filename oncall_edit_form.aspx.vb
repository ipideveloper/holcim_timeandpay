...
Partial Class oncall_edit_form
...
#Region "Events"
    ...
    Protected Sub BtnSaveExtend_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnSaveExtend.Click
        ...
        'UserMsgBox(ViewState("pressed").ToString)
        Try
            If ViewState("pressed") = "True" Then
                ViewState("pressed") = "True"
                ...
                Try
                    If ws.Validate_Duplicate_DateTime(Session("RefNo"), IIf(dplEmployee.SelectedIndex = 0, Current_User.Employee_ID, dplEmployee.SelectedValue), CType(v_from, Date), CType(v_to, Date), "OC") = True Then
                ...
                Session("Employee_ID") = ViewState("Employee_ID")
                BtnSaveExtend.Enabled = True
                'Response.Redirect(Session("Return_OnCall")) '"my_app_status.aspx")
                Response.Redirect("oncall.aspx")
            End If
        Catch ex As Exception
    ...
    Protected Sub BtnNO_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnNO.Click
        Try
            If ViewState("pressed") = "" Then
                ...
                If ws.Validate_Duplicate_DateTime(Session("RefNo"), dplEmployee.SelectedItem.Text, CType(v_from, Date), CType(v_to, Date), "OC") = True Then
                    UserMsgBox("You have filed the same date with your previous application.")
...
#Region "Initialization"
    ...
    Private Sub Initialize()
        ...
        get_employee_list()
        get_oncall_details(Session("RefNo"))
    End Sub
...
#Region "Insert"
    ...
    Private Sub insert_oncall_applications()
        Try
            ...
            For i As Integer = 0 To GridView1.Rows.Count - 1
                If i = 0 Then
                    ref_no = ws.Update_Oncall_Header(Session("RefNo"), GridView1.Rows(i).Cells(1).Text, _
                    ...
                    'UserMsgBox("Updated!")
                Else
                    ref_no = ws.Insert_Oncall_Header(GridView1.Rows(i).Cells(1).Text, _
                                                              CType(GridView1.Rows(i).Cells(3).Text, Date), _
                                                             GridView1.Rows(i).Cells(4).Text, _
                                                             GridView1.Rows(i).Cells(5).Text, _
                                                             GridView1.Rows(i).Cells(6).Text, _
    ...
    Private Sub add_employee_to_list()
        ...
        dr("time_to") = txt_date_from.Text & " " & dplTo.SelectedItem.Text & ":" & lstTo.SelectedItem.Text & " " & ampmTo.SelectedItem.Text
...
#Region "Retrieve"
    ...
    Private Sub get_oncall_details(ByVal ref_no As String)
        Try

            Dim dr As DataRow = ws.Get_Oncall_Details(ref_no).Tables(0).Rows(0)
            With dr
                ...
                dplEmployee.Enabled = False
                txt_date_from.Text = dr("datefrom")
                txt_date_to.Text = dr("dateto")
                txt_contact_by.Text = dr("contacted_by")
                txt_details.Text = dr("details")

                'dplFrom.SelectedIndex = Hour(dr("date_time_from")) - 12 - 1
                'dplTo.SelectedIndex = Hour(dr("date_time_to")) - 12 - 1
                dplFrom.SelectedIndex = IIf(Hour(dr("date_time_from")) > 12, Hour(dr("date_time_from")) - 12 - 1, IIf(Hour(dr("date_time_from")) = 12, Hour(dr("date_time_from")), Hour(dr("date_time_from")) - 1))
                dplTo.SelectedIndex = IIf(Hour(dr("date_time_to")) > 12, Hour(dr("date_time_to")) - 12 - 1, IIf(Hour(dr("date_time_to")) = 12, Hour(dr("date_time_to")), Hour(dr("date_time_to")) - 1))

                lstFrom.Text = Minute(dr("date_time_from"))
                lstTo.Text = Minute(dr("date_time_to"))
                ampmFrom.Text = Right(CType(dr("date_time_from"), String), 2)
                ampmTo.Text = Right(CType(dr("date_time_to"), String), 2)
...
#Region "Javascripts"
    ...
    Private Sub send_notifications(ByVal ref_no As String, ByVal employee_id As String, ByVal employee_name As String)
        ...
        body = employee_name & " is applying for ON CALL (Edited). For your approval, please see Ref. " & ref_no & " <br><br><br><br> This is a system-generated message.  Do not reply to this message."
        Dim ds As DataSet, level As String, r_level As String

        r_level = ws.Get_approvers_level(dplEmployee.SelectedValue, Current_User.Employee_ID)
