    ...
    Protected Sub GridView1_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridView1.RowCommand
        Try
            ...
            If e.CommandName = "cmdReset" Then
                ...
                'Dim killbibing As New System.Text.StringBuilder
                'killbibing.Append("<script language='javascript'>")
                'killbibing.Append("alert('Gago');")
                'killbibing.Append("</script>")
                'Response.Redirect("message.aspx")
                'MsgBox("Reset Password Success!")
                '--------msgbox start
                'Msg Function code start
                'Dim someScript As String = "AlertMe"
                'If (Not ClientScript.IsOnSubmitStatementRegistered(Me.GetType(), someScript)) Then
                'ClientScript.RegisterStartupScript(Me.GetType(), someScript, "alert('Resetting of password successfully done')", True)
                'End If
                'Dim strScript As String = "<script language='javascript' id='myClientScript'>Alert();</script>"
                'ClientScript.RegisterStartupScript(Me.GetType(), "callTest", strScript)
                'Msg function code end
